...
ollama_llm = ChatOllama(
    ...
    temperature=0.3,
    num_ctx=32768,
    client_kwargs={"headers": {"Authorization": f"Bearer {env.OLLAMA_API_KEY}"}},
)
...
def invoke_llm_chain(messages):
    ...
    # return None
    try:
        logger.info("🏠 Đang gọi Ollama (Streaming)...")

        # Dùng .stream() thay vì .invoke()
        response_stream = ollama_llm.stream(messages)

        full_response = ""

        # Duyệt qua từng chunk trả về từ Ollama
        for chunk in response_stream:
            # In ra màn hình ngay lập tức (nếu bạn chạy trên terminal)
            print(chunk.content, end="", flush=True)

            # Ghép vào chuỗi kết quả cuối cùng
